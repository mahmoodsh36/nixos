  ...
  config = lib.mkMerge [
    (lib.mkIf config.machine.enable_nvidia {
      ...
      systemd.services.llamacpp_embed_service = {
        ...
        wantedBy = [ "multi-user.target" ];
        script = "${inputs.llama-cpp-flake.packages.${pkgs.system}.cuda}/bin/llama-server --host 0.0.0.0 --port 5001 -hf Qwen/Qwen3-Embedding-0.6B-GGUF:Q8_0 -ngl 99 -fa -c 16000 --seed 2 --rope-scaling yarn --rope-freq-scale .75 --embedding --ubatch-size 2000";
        serviceConfig = {
          Restart = "always";
        };
        unitConfig = {
