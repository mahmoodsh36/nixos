...
if [ "$conf" == "droid" ]; then
    ...
    sudo darwin-rebuild switch --flake .#mahmooz0 --show-trace --impure $extra_args --keep-going
elif [ "$conf" == "mahmooz5" ]; then
    sudo -E nixos-rebuild switch --flake .#"$conf" --show-trace --impure $extra_args
else
