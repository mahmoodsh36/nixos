...
in
{
  config = {
    ...
    # tailscaled on macos only registers *.ts.net in the system dns store, so
    # our headscale suffix (tailnet.<domain>) never resolves without this.
    # /etc/resolver files written by tailscaled are ignored on macos 26.
    # dns itself comes from services.tailscale.overrideLocalDns, do not set it here.
    networking.search = [ "tailnet.${constants.mydomain}" ];
    ...
    nix = {
      linux-builder = {
        ...
        config = {
          ...
          # step only when the clock is actually wrong: hwclock --hctosys sets it every time,
          # and moving it back even a few ms dates a running build's outputs in the future.
          systemd.services.rtc-resync = {
            startAt = "minutely";
            after = [ "time-set.target" ];
            script = ''
              sw=/run/current-system/sw/bin
              rtc=$($sw/date -d "$($sw/hwclock --show)" +%s)
              now=$($sw/date +%s)
              if [ $((rtc - now)) -gt 2 ] || [ $((now - rtc)) -gt 2 ]; then
                $sw/hwclock --hctosys
              fi
            '';
